...
class CRFSequenceLabeler(SequenceLabeler):

    def __init__(self, id2label):
        super(CRFSequenceLabeler, self).__init__()
        self.id2label = id2label
        self.crf = CRF(num_tags=len(id2label) - 1, batch_first=True)

    def forward(self,
                logits: torch.Tensor,
                mask: torch.Tensor,
                tags: torch.Tensor):
        return -1 * self.crf(logits, tags, mask=mask.byte(), reduction='mean')
