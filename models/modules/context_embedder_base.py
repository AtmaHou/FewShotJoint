...
import logging
import sys
from transformers import BertModel, ElectraModel, BertConfig, RobertaModel
...
class BertContextEmbedder(ContextEmbedderBase):
    ...
    def forward(
        ...
        if support_token_ids is not None:
            return self.concatenate_reps(
                ...
                support_token_ids, support_segment_ids, support_nwp_index, support_input_mask, use_cls
            )
        else:
            return self.single_reps(test_token_ids, test_segment_ids, test_nwp_index, test_input_mask, use_cls)
    ...
    def concatenate_reps(
        ...
        if use_cls:
            # use cls
            test_sent_reps = sequence_output.narrow(-2, 0, 1).view(batch_size, support_size, -1, reps_size)  # (batch, 1, reps_size)
            # use sep
            support_sent_reps = sequence_output.narrow(-2, 1 + test_len, 1).view(batch_size, support_size, -1, reps_size)  # (batch, 1, reps_size)
        else:
    ...
    def single_reps(
        ...
        ''' get bert reps '''
        origin_shape = list(test_token_ids.shape)
        test_token_ids = test_token_ids.contiguous().view(-1, test_len+2)
        test_input_mask = test_input_mask.contiguous().view(-1, test_len+2)
        test_segment_ids = test_segment_ids.contiguous().view(-1, test_len+2)
        test_sequence_output = self.embedder(test_token_ids, test_input_mask, test_segment_ids)[0]
        origin_shape.append(test_sequence_output.size(-1))
        test_sequence_output = test_sequence_output.reshape(*origin_shape)
        ''' extract reps '''
        # select pure sent part, remove [SEP] and [CLS], notice: seq_len1 == seq_len2 == max_len.
        test_reps = test_sequence_output.narrow(-2, 1, test_len)  # shape:(batch, test_len, rep_size)
        # test_reps = test_reps.reshape(*origin_shape)
        # select non-word-piece tokens' representation
        nwp_test_reps = self.extract_non_word_piece_reps(test_reps, test_nwp_index)
        if use_cls:
            test_sent_reps = test_sequence_output.narrow(-2, 0, 1)
            if len(test_sent_reps.size()) == 3:
                test_sent_reps = test_sent_reps.unsqueeze(1)
        else:
            # get whole word reps, unsqueeze to fit interface
...
class BertSchemaContextEmbedder(BertContextEmbedder):
    ...
    def forward(
        ...
        if reps_type == 'test_support':
            ...
            return self.get_label_reps(test_token_ids, test_segment_ids, test_nwp_index, test_input_mask, use_cls)

    def get_label_reps(self, test_token_ids, test_segment_ids, test_nwp_index, test_input_mask, use_cls=False):
        batch_size = test_token_ids.shape[0]
        if self.opt.label_reps == 'cat':
            reps = self.single_reps(test_token_ids, test_segment_ids, test_nwp_index, test_input_mask, use_cls)
...
class BertSeparateContextEmbedder(BertContextEmbedder):
    ...
    def forward(
        ...
        if support_token_ids is not None:
            return self.separate_reps(
                test_token_ids, test_segment_ids, test_nwp_index, test_input_mask,
                support_token_ids, support_segment_ids, support_nwp_index, support_input_mask, use_cls=use_cls
            )
        else:
            return self.single_reps(test_token_ids, test_segment_ids, test_nwp_index, test_input_mask, use_cls=use_cls)
    ...
    def separate_reps(
        ...
        if use_cls:
            test_sent_reps = test_sequence_output.narrow(-2, 1, 1)
            test_sent_reps = test_sent_reps.unsqueeze(-2)
            support_sent_reps = support_sequence_output.narrow(-2, 1, 1).view(batch_size, support_size, -1, reps_size)
        else:
            # get whole sent reps
            test_sent_reps = self.get_sent_reps(test_reps, test_input_mask)
            support_sent_reps = self.get_sent_reps(support_reps, support_input_mask)
...
class RobertaContextEmbedder(BertContextEmbedder):
    """ Electra based context embedder """
    def __init__(self, opt):
        super(RobertaContextEmbedder, self).__init__(opt)

    def build_embedder(self):
        """ Load pretrained params """
        return RobertaModel.from_pretrained(self.opt.bert_path)
    ...
    def forward(
        ...
        # set segment ids all to zero in roberta
        test_segment_ids = torch.zeros(test_segment_ids.size()).long().to(test_segment_ids.device)
        support_segment_ids = torch.zeros(support_segment_ids.size()).long().to(support_segment_ids.device)
