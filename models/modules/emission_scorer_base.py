...
class PrototypeEmissionScorer(EmissionScorerBase):
    ...
    def forward(
        ...
        :return: emission, shape: (batch_size, test_len, no_pad_num_tags)
        """
        similarity = self.similarity_scorer.cal_similarity(test_reps)
        emission = self.get_emission(similarity, support_targets)  # shape(batch_size, test_len, no_pad_num_tag)
        return emission
    ...
    def cal_prototype(
        ...
        self.prototype = self.similarity_scorer(
            test_reps, support_reps, test_output_mask, support_output_mask, support_targets)
        return self.prototype

    def update_prototype(self, prototype_reps):
        self.prototype = self.similarity_scorer.update_prototype(prototype_reps)
        return self.prototype

    def set_prototype(self, prototype_reps):
        self.similarity_scorer.set_prototype(prototype_reps)

    def get_prototype(self):
        return self.similarity_scorer.get_prototype()
...
class ProtoWithLabelEmissionScorer(PrototypeEmissionScorer):
